...
function OptimizadorProductos() {
  ...
  const [porcentajeMutacion, setPorcentajeMutacion] = useState(0.2);
  const [poblacion, setPoblacion] = useState(50);
  const [generaciones, setGeneraciones] = useState(300);
  const [volumenEstante, setVolumenMaximo] = useState(10000);
  const [alpha, setAlpha] = useState(10);
  const [filasEstante, setFilasEstante] = useState(2);
  ...
  .map(product => ({
    ...
    try {
      ...
      const response = await axios.post('http://127.0.0.1:5000/algorithm/', {
        ...
        productosSeleccionados: productosEnviar
      });
      ...
      {showModal && (
        <div className="modal-overlay">
          <div className="modal">
            ...
            <label>
              Filas del Estante:
              <input type="number" value={filasEstante} onChange={(e) => setFilasEstante(parseInt(e.target.value))} />
            </label>
...
<Shelf
  ...
  numRows={filasEstante}
  volumenMaximoPorFila={volumenEstante / filasEstante}
/>
